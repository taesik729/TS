...
- **기본 화면 = 조회 모드**: 좌측 날짜·분류 필터 + 상단 제목/내용 통합 검색 + 그리드(읽기전용) + 하단 요청사항/처리사항(읽기전용 텍스트, textarea 아님). 행 클릭 시 하단에 해당 내용 표시만 됨 — 편집 불가
- **"추가" 클릭 시에만** 편집 폼으로 전환: 그리드 최상단에 인라인 입력 행(날짜/분류/제목) + 하단 요청사항/처리사항이 editable textarea로 바뀜. "수정"도 동일하게 선택된 행이 편집 폼으로 전환
