...
- **행 클릭 또는 "추가" 클릭 → 화면 우측에서 슬라이드로 열리는 패널(1120px 폭)**에서 작성/수정 (TS처럼 중앙 모달이 아니라 우측 패널 방식)
  - 패널 내부도 좌우 2단: **`.panel-main`(본문 영역, 800px)** + **`.panel-side`(댓글 전용 사이드, 320px, 우측 border로 구분)**
  - 본문(`.panel-main`): 업무명, 상태(진행/완료), 담당자, 시작일/마감일, 우선순위(낮음/보통/높음), 진척도(0~100) — 라벨을 입력창 위가 아니라 옆에 배치("상태: [콤보]" 형태, `.meta-row`), 그 아래 리치텍스트(TipTap) 본문
  - 본문은 **요청사항/처리사항 구분 없이 하나의 리치텍스트** — `.editor-section`/`.editor-content`가 flex:1이라 남은 세로 공간을 그대로 채움(고정 높이 아님). 붙여넣기·드래그로 이미지 넣으면 `csr-attachments` Storage에 자동 업로드 후 본문에 바로 삽입됨
  - **댓글**(`.panel-side`)은 업무를 먼저 저장(신규 생성)해야 추가 가능 — `task_id`가 있어야 하는 하위 테이블이라 저장 전에는 "저장 후 추가 가능" 안내만 표시. 댓글별 삭제 버튼 있음(`useComments.deleteComment`)
