...
- **본문 내 첫 매칭 위치로 자동 스크롤 이동** — `scheduleContentHighlight()`의 마지막 재시도 프레임에서만 `applyContentHighlight(true)`를 호출해 스크롤까지 실행(중간 프레임은 하이라이트만). `scrollToRange()`가 `.editor-content` 컨테이너 기준으로 (현재 스크롤 위치와 무관하게) 매칭 Range의 절대 위치를 계산해서 화면 중앙쯤에 오도록 `scrollTop`을 직접 대입 — **`scrollTo({behavior:'smooth'})`는 사용하지 않음**(환경에 따라 스무스 스크롤 애니메이션 자체가 실행되지 않고 무시되는 경우가 확인되어, 즉시 이동 방식으로 구현)
...
- **`BottomNav`는 `position: fixed; bottom: 0`으로 뷰포트에 완전히 고정** — 처음엔 flex 레이아웃(`min-height:0` 체인)만으로 하단 고정을 시도했으나, 실기기에서 미세한 오차로 계속 화면과 같이 스크롤되는 문제가 반복 신고됨 → 근본 원인을 계속 찾기보다 `position:fixed`로 완전히 뷰포트에 못박는 방식으로 전환(더 견고함). 대신 `App.vue`의 `.page`에 `padding-bottom: calc(72px + env(safe-area-inset-bottom))`을 줘서 각 화면 콘텐츠 맨 아래가 고정 네비에 가려지지 않게 함. 네비 높이(72px) 바꾸면 이 패딩값도 같이 바꿀 것
- **각 화면(TS/CSR/업무일지/설정&분석)에 `@media (max-width: 720px)`** 반응형 규칙 — 좌측 고정폭 사이드바(`.side`, `.an-tree`)를 세로 스택 상단 배치로 전환, 슬라이드 패널은 `width: 100%`, 달력 셀은 패딩/폰트 축소. `src/style.css`에 `overflow-x: hidden` 안전망도 있음
- **모바일 필터바/툴바는 줄바꿈 대신 한 줄 + 가로 스크롤**: `.side`/`.toolbar`/`.an-toolbar`를 `flex-wrap: nowrap; overflow-x: auto`로, 내부 폰트/패딩도 축소(`.date-full` 너비 102px). **주의**: TS/CSR의 `.field`는 desktop에서 `flex-direction: column`(라벨 위, 입력 아래)인데 모바일에서 이걸 안 바꾸면 기간 필드(날짜~날짜+전체기간버튼)가 세로로 쌓여서 눈에 "줄바꿈"처럼 보이는 버그가 생김 — 모바일 미디어쿼리에서 `.field`도 반드시 `flex-direction: row; align-items: center`로 같이 바꿔줄 것
- **주의 2**: `.clear`(전체 기간 버튼)는 desktop CSS에 `align-self: flex-start`가 박혀 있어서, 모바일 미디어쿼리에서 명시적으로 `align-self: center`로 덮어쓰지 않으면 다른 요소들과 나란히 안 맞고 위로 떠 보임. `.date-full` 너비가 너무 좁으면(82px 이하) 날짜 텍스트가 잘리므로 100px 이상 유지할 것
