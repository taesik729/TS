...
- **검색어와 일치하는 트리 제목 글자는 노란색(`#fde047`)으로 하이라이트** — `AnalysisTreeNode`에 `keyword` prop을 재귀로 내려주고, 대소문자 무시하고 매칭되는 부분을 `<mark class="hl">`로 감싸서 `v-html` 렌더링(제목은 `escapeHtml`로 이스케이프 후 매칭 부분만 태그 삽입, XSS 방지). **트리 제목만** 하이라이트되고 우측 리치텍스트 본문 안쪽은 하이라이트 안 됨(편집 가능한 TipTap 문서라 저장 데이터를 건드리지 않기 위해 제외)
